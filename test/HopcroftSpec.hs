...
spec =
  describe "The dfa2min function" $ do
    ...
    it "should create a minimal dfa from the dfa of \"a|b\"" $
      ...
      { dfaSigma =
          Set.fromList
            [((DFAState 0, 'a'), DFAState 1), ((DFAState 0, 'b'), DFAState 1)]
      , dfaStart = DFAState 0
      , dfaAcceptingStates = Set.singleton (DFAState 1)
      }
    it "should create a minimal dfa from the dfa of \"a*\"" $
    ...
    it "should create a minimal dfa from the dfa of \"(a|b)*\"" $
      ...
      { dfaSigma =
          Set.fromList
            [((DFAState 0, 'a'), DFAState 0), ((DFAState 0, 'b'), DFAState 0)]
      , dfaStart = DFAState 0
      , dfaAcceptingStates = Set.singleton (DFAState 0)
      }
    it "should create a minimal dfa from the dfa of \"a|b*\"" $
    ...
    it "should create a minimal dfa from the dfa of \"fee|fie\"" $
      dfa2min
        ...
        , dfaStart = DFAState 0
        , dfaAcceptingStates = Set.fromList [DFAState 3, DFAState 5]
        } `shouldBe`
      DFA
      { dfaSigma =
          Set.fromList
    ...
    it "should create a minimal dfa from the dfa of \"a(b|c*)\"" $
      dfa2min
        ...
        , dfaStart = DFAState 0
        , dfaAcceptingStates = Set.fromList [DFAState 1, DFAState 2, DFAState 3]
        } `shouldBe`
      DFA
      { dfaSigma =
          Set.fromList
