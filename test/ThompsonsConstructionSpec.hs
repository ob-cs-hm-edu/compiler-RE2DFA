module ThompsonsConstructionSpec
  ( spec
  ) where
...
spec =
  describe "The re2nfa function" $ do
    ...
    it "should create a nfa from the regex of \"(a|b)*\"" $
      re2nfa (ClosureRE (AlternativeRE (PrimitiveRE 'a') (PrimitiveRE 'b'))) `shouldBe`
      NFA
      { nfaSigma =
          Set.fromList
            [ ((NFAState 0, Nothing), NFAState 1)
    ...
    it "should create a nfa from the regex of \"a|b*\"" $
      re2nfa (AlternativeRE (PrimitiveRE 'a') (ClosureRE (PrimitiveRE 'b'))) `shouldBe`
      NFA
      { nfaSigma =
          Set.fromList
            [ ((NFAState 0, Nothing), NFAState 1)
        ...
        (AlternativeRE
           (ConcatenatedRE
              ...
              (ConcatenatedRE
                 ...
                 (ConcatenatedRE (PrimitiveRE 'b') (PrimitiveRE 'd'))))
           (ConcatenatedRE (ClosureRE (PrimitiveRE 'b')) (PrimitiveRE 'e'))) `shouldBe`
      NFA
      { nfaSigma =
          Set.fromList
            [ ((NFAState 0, Nothing), NFAState 1)
