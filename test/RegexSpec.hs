...
spec =
  ...
  describe "The str2re function" $ do
    ...
    it "should create a Regex from \"ab\"" $
        str2re "ab" `shouldBe` Just (ConcatenatedRE (PrimitiveRE 'a')
                                                    (PrimitiveRE 'b')
                                        )

    it "should create a Regex from \"ab*\"" $
        str2re "ab*" `shouldBe`
          Just (ConcatenatedRE (PrimitiveRE 'a')
                               (ClosureRE (PrimitiveRE 'b'))
               )

    it "should create a Regex from \"(ab)*\"" $
        str2re "(ab)*" `shouldBe`
          Just (ClosureRE (ConcatenatedRE (PrimitiveRE 'a')
                                          (PrimitiveRE 'b')
                          )
               )

    it "should create a Regex from \"a|b\"" $
        str2re "a|b" `shouldBe` Just (AlternativeRE (PrimitiveRE 'a')
                                                        (PrimitiveRE 'b')
                                         )

    it "should create a Regex from \"ac|b\"" $
        str2re "ac|b" `shouldBe` Just (AlternativeRE
                                          (ConcatenatedRE
                                            (PrimitiveRE 'a')
                                            (PrimitiveRE 'c')
                                          )
    ...
    it "should create a Regex from \"a|b*\"" $
        str2re "a|b*" `shouldBe`
          Just (AlternativeRE (PrimitiveRE 'a')
                              (ClosureRE (PrimitiveRE 'b'))
               )

    it "should create a Regex from \"(a|b)*\"" $
        str2re "(a|b)*" `shouldBe`
          Just (ClosureRE (AlternativeRE (PrimitiveRE 'a')
                                         (PrimitiveRE 'b')
                          )
               )
