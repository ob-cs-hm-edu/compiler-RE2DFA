...
re2nfa' ::
     Integer -- zu vergebende Nummer für den Startzustand.
  -> RE
  -> NFA
re2nfa' n (PrimitiveRE c) =
  NFA
...
re2nfa' n (ConcatenatedRE re1 re2) =
  ...
  in NFA
     { nfaSigma =
         nfaSigma nfa1 `Set.union` nfaSigma nfa2 `Set.union`
         Set.map (\s -> ((s, Nothing), nfaStart nfa2)) (nfaAcceptingStates nfa1)
...
re2nfa' n (ClosureRE re) =
  ...
  in NFA
     { nfaSigma =
         nfaSigma nfa `Set.union`
         Set.map (\s -> ((s, Nothing), stateEnd)) (nfaAcceptingStates nfa) `Set.union`
         Set.map (\s -> ((s, Nothing), state1)) (nfaAcceptingStates nfa) `Set.union`
         Set.fromList
           [((state0, Nothing), state1), ((state0, Nothing), stateEnd)]
     , nfaStart = state0
...
re2nfa' n (AlternativeRE re1 re2) =
  ...
  in NFA
     { nfaSigma =
         ...
         Set.map
           (\s -> ((s, Nothing), stateEnd))
           (nfaAcceptingStates nfa1 `Set.union` nfaAcceptingStates nfa2)
     , nfaStart = NFAState n
     , nfaAcceptingStates = Set.singleton stateEnd
     }
