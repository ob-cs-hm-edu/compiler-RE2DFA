...
delta states nfa c =
  ...
  in Set.map snd $ Set.filter ((`Set.member` args) . fst) $ nfaSigma nfa

epsilonClosure :: Set.Set NFAState -> NFA -> Set.Set NFAState
epsilonClosure reachedStates nfa =
  let newStates =
        Set.union reachedStates $
        Set.map snd $
        Set.filter
          (\((from, with), _) -> isNothing with && from `elem` reachedStates) $
        nfaSigma nfa
...
dfaFromQandT nfa (q, t) =
  let nfaAndDfaStates =
      ...
      dfaSigmaFromT =
        Set.map
          (\((fromStates, c), toStates) ->
             ( (fromJust $ Map.lookup fromStates nfaAndDfaStates, c)
             , fromJust $ Map.lookup toStates nfaAndDfaStates))
          t
  in DFA
     ...
     , dfaStart =
         fromJust $
         Map.lookup
           (epsilonClosure (Set.singleton $ nfaStart nfa) nfa)
           nfaAndDfaStates
     , dfaAcceptingStates =
         Set.fromList $
         Map.elems $
         (\s -> Map.filterWithKey (\k _ -> k `Set.member` s) nfaAndDfaStates) $
         Set.filter
           (not . Set.null . Set.intersection (nfaAcceptingStates nfa))
           q
     }
