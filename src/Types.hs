...
dfaStates dfa =
              Set.singleton (dfaStart dfa)
  `Set.union` dfaAcceptingStates dfa
  `Set.union` Set.unions
                ( map (\((from,_),to) -> Set.fromList [from, to])
                  $ Set.toList $ dfaSigma dfa)

dfaBigSigma :: DFA -> String
dfaBigSigma = Set.toList
            . Set.map (snd . fst)
            . dfaSigma
