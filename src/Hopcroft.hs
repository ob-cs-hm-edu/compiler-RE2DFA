...
initialT dfa =
  ...
  Set.fromList
    [dfaAcceptingStates dfa, dfaStates dfa Set.\\ dfaAcceptingStates dfa]
...
applySigmaToChar dfa state char =
  Set.map snd $
  Set.filter (\((from, c), _) -> from == state && c == char) $ dfaSigma dfa
...
mkDFA dfa t =
  let oldAndNewStates = zip (Set.toList t) $ map DFAState [0 ..]
      newStateFromOld s =
        snd $
        head $ filter (\(states, _) -> s `Set.member` states) oldAndNewStates
  in DFA
     { dfaSigma =
         Set.map
           (\((from, char), to) ->
     ...
     , dfaAcceptingStates =
         ...
         map snd $
         filter
           (not . null . Set.intersection (dfaAcceptingStates dfa) . fst)
           oldAndNewStates
     }
